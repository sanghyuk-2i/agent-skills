...
이 스킬은 "쏙쏙 들어오는 함수형 코딩"의 원칙을 React + TypeScript 코드에 적용한다. 함수형 **언어**를 쓰자는 게 아니라, 액션과 계산을 나눠 코드를 단순하게 만드는 실용적 방법이다.
...
`assets/CLAUDE.md.snippet`을 프로젝트의 `CLAUDE.md`에 붙인다. 이래야 이 스킬 없이 작업하는 세션에서도 구조가 유지된다.
