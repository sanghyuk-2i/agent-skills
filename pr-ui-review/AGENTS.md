# pr-ui-review — PR UI 리뷰 스크린샷

GitHub PR의 UI 변경을 before/after 스크린샷으로 비교하고, 실제로 달라진 영역에만 번호 붙은 박스를 그려 PR 코멘트로 게시하는 워크플로우다. base 브랜치를 git worktree로 띄워 현재 브랜치와 같은 화면을 캡처하고, 픽셀 diff로 변경 영역을 찾은 뒤 각 박스에 설명을 단다.

**언제 쓰나**: PR을 올릴 때, UI/스타일/컴포넌트를 수정했을 때, "화면 뭐가 바뀌었는지 보여줘", "스크린샷 붙여줘", "리뷰어가 보기 쉽게" 같은 요청. 사용자가 "스크린샷"이라는 말을 쓰지 않아도 프론트엔드 변경을 PR로 올리는 맥락이면 사용한다.

코드 diff는 `className` 한 줄이 화면에서 무엇을 바꾸는지 알려주지 않는다. 이 워크플로우는 base와 head를 **동시에 띄워 같은 화면을 찍고**, 픽셀 비교로 달라진 곳에만 박스를 그려 PR 코멘트로 올린다.
...
|---|---|
| 매칭된 라우트 있음 | 그대로 진행 |
| 설정은 있는데 매칭 0개 | 변경 파일을 import 그래프로 역추적해 후보 라우트를 찾고, **사용자에게 질문해 확인한다** |
...
`<name>.box-N.png`(해당 영역의 before/after 크롭)를 **직접 열어서 확인하고** 코드 diff와 대조해 한 줄씩 쓴다. 이미지를 열어볼 수 없는 환경이면, 크롭 이미지 경로를 사용자에게 제시하고 눈으로 확인한 내용을 알려달라고 요청한 뒤 그 설명을 반영한다 — 크롭을 보지 않고 코드 diff만으로 추측해서 쓰지 않는다.
...
| 도구 | 진입점 | 비고 |
|---|---|---|
| Claude Code | `SKILL.md` (frontmatter로 자동 트리거) | 이 문서를 그대로 읽고 따르되, "사용자에게 질문해 확인"은 `AskUserQuestion` 도구로, 이미지 확인은 `Read` 도구로 수행 |
| Cursor | `.cursor-rule.mdc`를 대상 프로젝트의 `.cursor/rules/`에 복사 | description 기반으로 관련 있을 때 자동 첨부됨(Agent Requested) |
| Codex CLI / 기타 | 이 파일을 직접 참조 | 조건부 자동 로딩이 없으므로, 필요한 시점에 "`pr-ui-review/AGENTS.md`를 읽고 그대로 따라줘"처럼 명시적으로 지시하거나, 대상 프로젝트의 `AGENTS.md`에 이 파일에 대한 링크를 남겨둔다 |
